...
function Login(props) {
  ...
  return (
    <div className="shadow-lg md:ml-4 border-2 w-[400px] px-4 py-2 flex justify-center items-center flex-col h-[350px] mt-4">
      <input
        type="text"
        className=" my-2 p-2 w-[280px] border rounded-[5px]"
        value={email}
        onChange={(event) => {
      ...
      <button onClick={login} className="mb-3 p-[3px] relative mt-3  ">
        ...
        <div className="relative px-8 py-2 text-black transition duration-200 bg-white rounded-full hover:scale-125 group hover:bg-transparent hover:text-white hover:font-bold">
          Login
        </div>
      </button>
      <p className="" onClick={change}>
        Dont have account ?{" "}
        <span className="font-bold cursor-pointer hover:text-purple-600">
          Register
        </span>
      </p>
    </div>
