...
export default function RegisterVerify(props) {
  ...
  return (
    <div className="grid place-items-center h-screen overflow-hidden">
      <div className="w-[80%] sm:w-1/2 md:w-1/3 lg:w-1/4 h-[80%] border border-slate-300 rounded-lg p-6 flex flex-col items-center justify-center ">
        <div className="py-4 text-2xl flex flex-col items-center justify-center">
          <h1 className="font-bold ">CoLab</h1>
          <p className="text-sm font-mono mb-7">Empower remote teamwork.</p>
          <p className="text-base">
            Email: <span className="font-semibold">{emailId}</span>
          </p>
        </div>
        {isVerified ? (
          <FinalRegister emailId={emailId} />
        ) : (
          <OtpVerify check={setVerified} generatedOtp={generatedOtp} />
        )}
      </div>
    </div>
  );
